class WebRtc {
    ...
    connectPeerOne() {
        ...
        dc.onopen = e => {console.log('WebRTC.js: Connection from Peer One Opened')};
        lc.onicecandidate = e => {console.log(JSON.stringify(lc.localDescription))};
        lc.createOffer().then(o => {
            lc.setLocalDescription(o)
        }).then(console.log('WebRTC.js: Local Description Seted Successfuly'));
    }

    connectPeerTwo() {
        const rc = new RTCPeerConnection();
        rc.onicecandidate = e => {console.log(rc.localDescription)}
        rc.ondatachannel = e => {
            rc.dc = e.channel;
            rc.dc.onmessage = e => console.log('new message from client' + e.data);
            rs.dc.onopen = e => console.log('connection open');
        }
